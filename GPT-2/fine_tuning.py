"""
The fine-tuning code is the same as the one provided by Huggingface, except for the 
Dataset Class.
    ...
    def __init__(self, tokenizer, args, file_path='train', block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            if type(tokenizer).__name__ == 'GPT2Tokenizer':
                print("Special tokenization for NLP Project")
                self.examples = []
                tokenized_text = []
                with open(file_path, encoding='utf-8') as f:
                    line = f.readline()
                    while line:
                        tokenized_text.append(tokenizer.convert_tokens_to_ids(tokenizer.tokenize(line)))
                        line = f.readline()
                
                # max size of joke sentence
                max_tokens = 50
                for i in range(len(tokenized_text)):
                    toks = tokenizer.build_inputs_with_special_tokens(tokenized_text[i])
                    eos = tokenizer.eos_token_id
                    if len(toks) > max_tokens:
                        print("one tokens greater than 50")
                        continue
                    toks = toks + [eos]*(max_tokens - len(toks))
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    print('file', args.train_data_file)
